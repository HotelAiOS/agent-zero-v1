"""
Agent Zero V1 - Dynamic Task Prioritization & Re-assignment - Point 3/6
Week 43 Implementation - Advanced AI Task Management
...
# Import from previous points
try:
    from context_aware_agent_selection_demo import (
        Task, TaskType, TaskPriority, AgentProfile, TaskAssignment, 
        TeamComposition, SelectionStrategy
    )
...
class DynamicTaskManager:
    ...
    def _initialize_default_rules(self):
        ...
        default_rules = [
            ...
            PrioritizationRule(
                ...
                condition="self.business_context.quarter_end_approaching and task.task_type in [TaskType.TESTING, TaskType.DEPLOYMENT]",
                priority_adjustment=1,
                weight=1.1
            )
        ]
    ...
    def update_business_context(self, context: BusinessContext):
        ...
        asyncio.create_task(self.evaluate_all_priorities())
    ...
    async def evaluate_reassignments(self) -> List[TaskReassignment]:
        ...
        overloaded_agents = [agent_id for agent_id, load in agent_workloads.items() if load > 1.2]
        underloaded_agents = [agent_id for agent_id, load in agent_workloads.items() if load < 0.6]
...
async def demo_dynamic_task_management():
    ...
    print_colored("🚀 Dynamic Task Prioritization & Re-assignment Demo", "cyan")
