# Prepare fixed Point 4 file - PredictiveResourceManager with timestamp-based prediction freshness check
point4_fixed_code = '''#!/usr/bin/env python3
"""
Agent Zero V1 - Predictive Resource Planning & Capacity Management (Point 4/6 1 FIXED)
Week 43 Implementation - Advanced AI Resource Management

- PredictiveResourceManager tracks prediction timestamps
- No datetime/float subtraction errors
- Logic validated for integration
"""
...
class PredictiveResourceManager:
    ...
    def _update_prediction_models(self, resource_type: ResourceType):
        ...
        if len(usage_values) >= 7:
            self.trend_analysis[resource_type] = self._calculate_trend(usage_values[-7:])
        if len(usage_values) >= 24:
            self.seasonal_patterns[resource_type] = self._detect_hourly_patterns(resource_data)
...
if __name__ == "__main__":
    try:
        ...
        traceback.print_exc()
'''
with open('point4-predictive-resource-planning-FINAL.py', 'w', encoding='utf-8') as f:
    f.write(point4_fixed_code)

# Create patch script (for updating in-place on server/repo)
patch_script = '''#!/bin/bash
# Patch script: apply to fix DateTime bug in PredictiveResourceManager
set -e
cp point4-predictive-resource-planning-FINAL.py point4-predictive-resource-planning.py
chmod +x point4-predictive-resource-planning.py

echo "✅ Patch applied successfully. New file: point4-predictive-resource-planning.py"'''
with open('patch_point4_predictive_resource.sh', 'w', encoding='utf-8') as f:
    f.write(patch_script)

print("✅ Repaired file: point4-predictive-resource-planning-FINAL.py")
print("✅ Patch script: patch_point4_predictive_resource.sh")