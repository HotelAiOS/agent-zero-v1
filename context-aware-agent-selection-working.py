"""
Agent Zero V1 - Context-Aware Agent Selection & Task Assignment - WORKING VERSION
Point 2 of 6 Critical AI Features - Week 43 Implementation

Inteligentny system wyboru agentów na podstawie kontekstu, expertise, workload i performance
"""
...
try:
    ...
    NLU_AVAILABLE = False
    print("⚠️ NLU components will use fallback mode")
    ...
    class NLUTaskDecomposer:
        async def enhanced_decompose(self, description: str, context: DomainContext) -> TaskBreakdown:
            ...
            tasks = [
                Task(1, "Architecture Design", f"Design architecture for {description[:30]}...", 
                     TaskType.ARCHITECTURE, TaskPriority.HIGH, 8.0, "backend"),
                Task(2, "Backend Implementation", f"Implement backend for {description[:30]}...", 
                     TaskType.BACKEND, TaskPriority.MEDIUM, 16.0, "backend"),
                Task(3, "Frontend Implementation", f"Implement frontend for {description[:30]}...", 
                     TaskType.FRONTEND, TaskPriority.MEDIUM, 12.0, "frontend"),
                Task(4, "Testing", f"Test {description[:30]}...", 
                     TaskType.TESTING, TaskPriority.MEDIUM, 8.0, "testing")
            ]
...
class ContextAwareAgentSelector:
    ...
    def _analyze_team_composition(
        ...
        team_utilization = sum(a.current_workload for a in agents) / sum(a.max_workload for a in agents) if agents else 0
        team_performance = sum(a.performance_history.get('last_month', 0.8) for a in agents) / len(agents) if agents else 0.8
    ...
    def _calculate_project_timeline(
        ...
        phase_duration = max(total_duration / len(phases), 1)
...
async def demo_context_aware_agent_selection():
    ...
    # Test scenario: Full-stack web application
    ...
    project_description = "Create comprehensive user management system with JWT authentication, role-based access control, React admin dashboard, and RESTful API endpoints"
