...
# GENEROWANIE PLIKU 3: Task Decomposer Fix
task_decomposer_code = '''"""
...
class RobustJSONParser:
    ...
    - Malformed JSON with recoverable errors
    ...
    def _parse_with_regex(text: str) -> Optional[Dict]:
        ...
        # Find content between outermost { }
        pattern = r'\\{[^{}]*(?:\\{[^{}]*\\}[^{}]*)*\\}'
    ...
    def _parse_with_fixes(text: str) -> Optional[Dict]:
        ...
        fixes = [
            ...
            lambda s: s.replace('True', 'true').replace('False', 'false'),  # Python booleans
        ]
...
class TaskDecomposer:
    ...
    FIXED: Robust JSON parsing from LLM responses (line ~220 issue)
    """
    ...
    async def decompose_task(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                # Generate decomposition with LLM
    ...
    def _build_prompt(
        ...
        """Build LLM prompt with emphasis on JSON format for retry attempts"""
...
"""
    ...
    async def _call_llm(self, prompt: str) -> str:
        ...
        # This would call actual LLM - placeholder for now
...
if __name__ == "__main__":
    ...
    test_cases = [
        '{"test": "direct"}',
        '```json\\n{"test": "code_block"}\\n```',
        'Some text before {"test": "with_text"} and after',
        "{'test': 'single_quotes'}",
        '{"test": "trailing_comma",}',
    ]
    ...
    for i, test in enumerate(test_cases):
        result = parser.extract_json(test)
        print(f"Test {i+1}: {'✅' if result else '❌'} - {result}")
'''

with open('task_decomposer_fixed.py', 'w', encoding='utf-8') as f:
    f.write(task_decomposer_code)

print("✅ Generated: task_decomposer_fixed.py")
