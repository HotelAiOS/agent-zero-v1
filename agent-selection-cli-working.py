#!/usr/bin/env python3
"""
Agent Zero V1 - Context-Aware Agent Selection CLI - WORKING VERSION
Week 43 - Point 2 Implementation - CLI Interface that works!
"""
...
try:
    # Load the working agent selection system
    exec(open(Path(__file__).parent / "context-aware-agent-selection-working.py").read())
...
def parse_args():
    ...
    if command == "select":
        if len(sys.argv) < 3:
            print("❌ Usage: python agent_selection_cli_working.py select \"project description\" [options]")
            return None
        ...
        if len(sys.argv) < 3:
            print("❌ Usage: python agent_selection_cli_working.py compare \"project description\" [options]")
            return None
...
Examples:
  python agent_selection_cli_working.py select "Create user auth system"
  
  python agent_selection_cli_working.py select "Build e-commerce API" \\
    --tech-stack FastAPI,PostgreSQL,Redis --project-type api_service \\
    --strategy expertise --max-team-size 6
    
  python agent_selection_cli_working.py compare "Create social media platform"
  
  python agent_selection_cli_working.py analyze
...
async def main():
    """Main CLI function"""
    console.print("🧠 Agent Zero V1 - Context-Aware Agent Selection - WORKING VERSION", "cyan")
    print("=" * 70)
