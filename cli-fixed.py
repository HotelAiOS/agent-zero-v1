#!/usr/bin/env python3
"""
Agent Zero V1 - FIXED Enhanced CLI Commands for NLU Task Decomposer
Week 43 - Production Ready Version with correct imports
"""
...
# Import NLU components with fallbacks
NLU_AVAILABLE = False
try:
    # Try to import from the file we just tested
    import nlp_enhanced_task_decomposer as nlu_module
    
    # Extract classes we need
    NLUTaskDecomposer = nlu_module.NLUTaskDecomposer
    DomainContext = nlu_module.DomainContext
    TaskBreakdown = nlu_module.TaskBreakdown
    TaskPriority = nlu_module.TaskPriority
    TaskType = nlu_module.TaskType
    Task = nlu_module.Task
    
    NLU_AVAILABLE = True
    print("✅ NLU Task Decomposer components imported successfully")
    ...
    class NLUTaskDecomposer:
        def __init__(self):
            print("⚠️ Using fallback NLU decomposer")
...
class EnhancedTaskCLI:
    """Enhanced CLI for NLU Task Decomposer with all fallbacks"""
    ...
    def _display_results(self, result: TaskBreakdown, original_description: str):
        ...
        if result.domain_knowledge:
            ...
            if "detected_technologies" in result.domain_knowledge:
                techs = result.domain_knowledge['detected_technologies']
                if techs:
                    print(f"  Technologies: {', '.join(techs)}")
            if "recommended_patterns" in result.domain_knowledge:
                patterns = result.domain_knowledge['recommended_patterns']
                if patterns:
...
def parse_args():
    ...
    if command == "analyze":
        if len(sys.argv) < 3:
            print("❌ Usage: python cli_fixed.py analyze \"task description\" [--tech-stack tech1,tech2] [--project-type type]")
            return None
        ...
        if len(sys.argv) < 5:
            print("❌ Usage: python cli_fixed.py export \"task description\" format filename")
            return None
...
def print_help():
    ...
    print("""
🚀 Agent Zero V1 - NLU Task Decomposer CLI (Fixed Version)

Commands:
  analyze "task description" [options]         Analyze task with AI
  export "task description" format filename   Export analysis results
...
Examples:
  python cli_fixed.py analyze "Create user auth system"
  
  python cli_fixed.py analyze "Build e-commerce API" \\
    --tech-stack FastAPI,PostgreSQL,Docker --project-type api_service
    
  python cli_fixed.py export "Create auth system" markdown auth_analysis.md
  
  python cli_fixed.py demo

Tech Stack Options:
  FastAPI, React, PostgreSQL, Neo4j, Docker, Redis
...
async def main():
    """Main CLI function"""
    print("🧠 Agent Zero V1 - Enhanced Task Decomposer CLI (Fixed)")
    print("=" * 65)
    ...
    if args["command"] == "analyze":
        ...
        result = await cli.analyze_task(args["description"])
        if result:
            cli.export_results(result, args["format"], args["filename"])
