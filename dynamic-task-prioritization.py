...
class DynamicPriorityEngine:
    ...
    async def _detect_crisis_indicators(self) -> List[Dict[str, Any]]:
        ...
        if recent_metrics and recent_metrics["avg_performance"] < 0.5:
            indicators.append({
                "crisis_type": CrisisType.PERFORMANCE_DEGRADATION,
                "description": f"System performance degraded to {recent_metrics['avg_performance']:.2f}",
                "severity": 2
            })
        ...
        overdue_critical = [
            ...
            if task.current_priority in [TaskPriority.CRISIS, TaskPriority.CRITICAL]
            and task.deadline and task.deadline < datetime.now()
        ]
        
        if len(overdue_critical) > 3:
            indicators.append({
                "crisis_type": CrisisType.CUSTOMER_ESCALATION,
                "description": f"{len(overdue_critical)} critical tasks overdue",
                "affected_tasks": [task.id for task in overdue_critical],
                "severity": 3
            })
        ...
        if self.agent_workloads:
            ...
            if max_workload > min_workload * 3:  # Severe imbalance
                indicators.append({
                    "crisis_type": CrisisType.RESOURCE_EXHAUSTION,
                    "description": f"Severe workload imbalance detected (max: {max_workload:.1f}h, min: {min_workload:.1f}h)",
                    "severity": 1
                })
    ...
    # (Store/retrieve from database, performance calculations, etc.)
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "dynamic_task_prioritization:app",
        host="0.0.0.0",
        port=8003,
        workers=1,
        log_level="info",
