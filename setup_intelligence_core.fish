#!/usr/bin/env fish
# Ustawienie katalogu repo
set REPO_DIR agent-zero-v1

# 1. Sklonuj, jeśli nie ma
...
# 3. Stwórz katalog AI i plik z mózgiem
mkdir -p app/ai
cat << 'EOF' > app/ai/intelligence_core.py
import os
import asyncio
import logging
...
class AgentZeroIntelligence:
    ...
    def __init__(self, neo4j_uri=None, neo4j_user=None, neo4j_password=None):
        ...
        self.models = {"quick":"llama3.1:8b","smart":"codellama:13b","expert":"qwen2.5-coder:32b"}
    ...
    async def think_and_execute(self, user_request: str) -> str:
        ...
        model = self.pattern_engine.optimize_model_choice(self.models[task_type], task_type)
    ...
    def _analyze_request(self, text: str) -> str:
        length = len(text)
        if length < 50: return "quick"
        if length < 200: return "smart"
        return "expert"

    async def _execute_with_ollama(self, model: str, prompt: str) -> str:
        try
            proc = await asyncio.create_subprocess_exec("ollama","run",model,"--prompt",prompt,stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE)
            out, err = await proc.communicate()
            if proc.returncode != 0: raise RuntimeError(err.decode().strip())
    ...
    def _learn_from_result(self, task_type: str, model: str, result: str):
        try
            with self.driver.session() as session
                session.write_transaction(self._create_record, task_type, model, result)
