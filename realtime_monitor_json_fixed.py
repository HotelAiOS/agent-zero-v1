#!/usr/bin/env python3
"""
🔧 Real-time Monitor V2.0 - JSON FIX
📦 PAKIET 5 Phase 2: JSON Serialization Fix
🎯 Fixes ValueError: Out of range float values are not JSON compliant: inf

Status: PRODUCTION READY - JSON FIXED
Created: 12 października 2025, 18:32 CEST
"""
...
def safe_float_for_json(value: float) -> float:
...
class Alert:
    ...
    def to_dict(self):
        ...
        return {
            ...
            'metric_value': safe_float_for_json(self.metric_value),
            'threshold_value': safe_float_for_json(self.threshold_value),
...
class PerformanceMetric:
    ...
    def to_dict(self):
        ...
        return {
            ...
            'current_value': safe_float_for_json(self.current_value),
            'previous_value': safe_float_for_json(self.previous_value) if self.previous_value is not None else None,
            'trend': self.trend,
            'threshold_warning': safe_float_for_json(self.threshold_warning),
            'threshold_critical': safe_float_for_json(self.threshold_critical),
            'unit': self.unit,
            'timestamp': self.timestamp.isoformat()
        }
...
class RealTimeMonitor:
    """
    📊 Real-time Performance Monitor for Agent Zero V2.0 - JSON FIXED
    ...
    def __init__(self):
        ...
        # Performance thresholds - FIXED: Use finite values for JSON
        self.thresholds = {
            ...
            'default': {'warning': 100, 'critical': 1000}
        }
        ...
        print(f"📊 Real-time Monitor initialized - JSON FIXED")
    ...
    async def start_monitoring(self):
        ...
        print("🚀 Starting Real-time Performance Monitor V2.0 - JSON FIXED")
    ...
    async def _collect_factory_metrics(self):
        ...
        try:
            ...
            if hasattr(self, '_last_task_count'):
                tasks_delta = max(0, total_tasks - self._last_task_count)  # Prevent negative
    ...
    async def _collect_kaizen_metrics(self):
        ...
        try:
            ...
            # Parse metrics from report with safe defaults
            alerts_count = report.get('alerts_count', 0)
            critical_alerts = report.get('critical_alerts', 0)
            
            self._update_metric("active_alerts", MetricType.AVAILABILITY, alerts_count, "count")
            self._update_metric("critical_alerts", MetricType.AVAILABILITY, critical_alerts, "count")
            
            # Cost analysis
            cost_analysis = self.kaizen_monitor.get_cost_analysis(days=1)
            total_cost = cost_analysis.get('total_cost_usd', 0)
            cost_efficiency = cost_analysis.get('cost_efficiency_score', 0)
            
            if total_cost > 0:
                self._update_metric("daily_cost", MetricType.COST, total_cost, "USD")
            if cost_efficiency > 0:
                self._update_metric("cost_efficiency", MetricType.COST, cost_efficiency, "ratio")
    ...
    def _update_metric(self, metric_name: str, metric_type: MetricType, 
        ...
        """Update a performance metric - JSON SAFE"""
        
        # Ensure value is JSON-safe
        value = safe_float_for_json(value)
        ...
        # Get thresholds - use defaults if not found
        thresholds = self.thresholds.get(metric_name, self.thresholds['default'])
        ...
        metric = PerformanceMetric(
            ...
            trend=trend,
            threshold_warning=safe_float_for_json(thresholds['warning']),
            threshold_critical=safe_float_for_json(thresholds['critical']),
            unit=unit,
    ...
    async def _create_alert(self, alert_type: str, severity: AlertSeverity,
        ...
        # Create alert with JSON-safe values
        alert = Alert(
            alert_id=alert_id,
            alert_type=alert_type,
            severity=severity,
            message=message,
            metric_value=safe_float_for_json(metric_value),
            threshold_value=safe_float_for_json(threshold_value),
    ...
    async def _store_alert_history(self, alert: Alert):
        ...
        try:
            ...
            ''', (
                ...
                alert.message,
                safe_float_for_json(alert.metric_value),
                safe_float_for_json(alert.threshold_value),
    ...
    async def _broadcast_metrics(self):
        ...
        # Prepare metrics data with JSON-safe values
        metrics_data = {
            'timestamp': datetime.now().isoformat(),
            'metrics': {
                name: {
                    'value': safe_float_for_json(metric.current_value),
                    'unit': metric.unit,
                    'trend': metric.trend,
                    'threshold_warning': safe_float_for_json(metric.threshold_warning),
                    'threshold_critical': safe_float_for_json(metric.threshold_critical)
                }
    ...
    def get_monitoring_dashboard_data(self) -> Dict[str, Any]:
        """Get comprehensive monitoring dashboard data - JSON SAFE"""
        
        global FACTORY_AVAILABLE, MONITORING_AVAILABLE, HAS_PSUTIL, HAS_FASTAPI
        
        # Helper function to get safe metric value
        def get_safe_metric_value(metric_name: str, default_value: float = 0.0) -> float:
            if metric_name in self.current_metrics:
                return safe_float_for_json(self.current_metrics[metric_name].current_value)
        ...
        return {
            ...
            'current_metrics': {
                name: metric.to_dict() for name, metric in self.current_metrics.items()
            },
            'active_alerts': {
                alert_id: alert.to_dict() for alert_id, alert in self.active_alerts.items()
            },
            'alert_summary': {
            ...
            'performance_summary': {
                'success_rate': get_safe_metric_value('success_rate', 0.0),
                'throughput': get_safe_metric_value('throughput_tpm', 0.0),
                'cost_efficiency': get_safe_metric_value('cost_efficiency', 0.0),
                'memory_usage': get_safe_metric_value('memory_usage_mb', 0.0),
                'cpu_usage': get_safe_metric_value('cpu_usage_percent', 0.0)
            },
            'last_update': datetime.now().isoformat()
        }

# FastAPI Web Dashboard (if available) - JSON FIXED
if HAS_FASTAPI:
    dashboard_app = FastAPI(
        title="Agent Zero V2.0 - Real-time Monitor - JSON FIXED",
    ...
    async def dashboard_home():
        """Monitoring dashboard home page - JSON FIXED"""
        
        html_content = """
        <!DOCTYPE html>
        <html>
        <head>
            <title>Agent Zero V2.0 - Real-time Monitor - JSON FIXED</title>
            <meta charset="utf-8">
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .success-indicator { position: fixed; top: 20px; left: 20px; background: #059669; color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.875rem; }
            </style>
        </head>
        <body>
            <div class="header">
                <div class="container">
                    ...
                    <p>Enterprise-grade monitoring with predictive analytics - JSON FIXED ✅</p>
                </div>
            </div>
            ...
            <div class="container">
                <div class="metrics-grid">
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-trend">System resources</div>
                    </div>
                    
                    <div class="metric-card">
                        <div class="metric-label">Cost Efficiency</div>
                        <div id="cost-efficiency" class="metric-value">Loading...</div>
                        <div class="metric-trend">Optimization score</div>
                    </div>
                </div>
            ...
            <div id="success-indicator" class="success-indicator">JSON Fixed ✅</div>
            ...
            <script>
                function updateMetrics(data) {
                    try {
                        ...
                        if (data.performance_summary) {
                            ...
                            const memoryUsage = data.performance_summary.memory_usage || 0;
                            const memoryElement = document.getElementById('memory-usage');
                            if (memoryElement) {
                                memoryElement.textContent = memoryUsage.toFixed(1) + ' MB';
                            }
                            
                            const costEfficiency = data.performance_summary.cost_efficiency || 0;
                            const costElement = document.getElementById('cost-efficiency');
                            if (costElement) {
                                costElement.textContent = (costEfficiency * 100).toFixed(1) + '%';
                            }
                        }
                        
                        // Update active alerts count
                        const alertsElement = document.getElementById('active-alerts');
                        if (alertsElement && data.alert_summary) {
                ...
                function fetchData() {
                    fetch('/api/dashboard')
                        ...
                        .then(data => {
                            updateMetrics(data);
                            console.log('Dashboard updated:', new Date().toLocaleTimeString(), 'Status:', data.system_status);
                        })
                        .catch(error => {
                ...
                console.log('🚀 Agent Zero V2.0 Real-time Monitor - JSON FIXED VERSION');
                console.log('📊 Live monitoring active - refresh every 5 seconds');
                console.log('✅ JSON serialization issues resolved');
            </script>
        </body>
        </html>
        """
    ...
    async def get_dashboard_data():
        """API endpoint for dashboard data - JSON SAFE"""
        global monitor_instance
        if monitor_instance:
            try:
                data = monitor_instance.get_monitoring_dashboard_data()
                # Double-check that all float values are JSON-safe
                return JSONResponse(content=data)
            except Exception as e:
                return JSONResponse(content={
                    "error": f"Monitor data error: {str(e)}",
                    "system_status": "error",
                    ...
                    "component_status": {
                        "enhanced_factory": "error",
                        "kaizen_analytics": "error",
                        "system_monitoring": "error",
            ...
            return JSONResponse(content={"error": "Monitor not initialized", "system_status": "error"})

# Export monitoring classes - JSON FIXED
__all__ = [
    'RealTimeMonitor',
    'Alert',
    'PerformanceMetric',
...
# Main execution for testing - JSON FIXED
if __name__ == "__main__":
    async def test_monitor():
        """Test real-time monitoring - JSON FIXED"""
        print("🚀 Testing Real-time Performance Monitor V2.0 - JSON FIXED")
    ...
    if HAS_FASTAPI:
        print("🌐 Starting Real-time Monitor Web Dashboard - JSON FIXED")
