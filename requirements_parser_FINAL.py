"""Agent Zero V1 - Business Requirements Parser with correct entity extraction, complexity, and costs"""
...
class BusinessRequirementsParser:
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        for ent in ["api","database","user"]:
            if re.search(rf"{ent}", tl):
                ents.append(ent)
        # report or reporting
        if re.search(r"report(ing|s)?", tl):
            ents.append("report")
        # authentication -> auth
    ...
    def generate_technical_spec(self, intent: str, entities: List[str], complexity: str, business_request: str) -> dict:
        ...
        if "api" in entities:
            tech_reqs["api"] = {"type":"REST","authentication":"JWT","documentation":"OpenAPI/Swagger"}
        if "database" in entities:
    ...
    def validate_request(self, request: str) -> dict:
        ...
        return {"is_valid": not errors, "errors": errors, "suggestions": suggestions, "confidence": conf}

    def sanitize_input(self, text: str) -> str:
        out = re.sub(r"<script.*?>.*?</script>", "", text, flags=re.IGNORECASE|re.DOTALL)
...
def parse_endpoint(req: BusinessRequest):
    ...
    if not v["is_valid"]:
        raise HTTPException(status_code=400, detail={"message":"Invalid business request","errors":v["errors"]})
    text = req.request
    intent = parser.parse_intent(text)
    entities = parser.extract_entities(text)
    complexity = parser.assess_complexity(text, entities)
    spec = parser.generate_technical_spec(intent, entities, complexity, text)
    return spec

@router.post("/validate", response_model=ValidationResponse)
def validate_endpoint(req: BusinessRequest):
    v = parser.validate_request(req.request)
    return ValidationResponse(**v)

@router.get("/health")
def health_check():
    return {"status":"healthy","service":"business_requirements_parser","version":"1.0.0","components":{"parser":True}}

@router.get("/capabilities")
def get_capabilities():
    return {"supported_intents":["CREATE","UPDATE"],"supported_entities":["api","database","user","report","auth"],"complexity_levels":["Simple","Moderate","Complex","Enterprise"],"features":{"validation":True}}
