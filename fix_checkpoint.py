...
# Znajdź metodę save_checkpoint i zamień na poprawioną wersję
old_method = r"""    async def save_checkpoint\(self, checkpoint: ProjectCheckpoint\) -> str:
        \"\"\"Save project checkpoint to disk\"\"\"
        import os
        os\.makedirs\(self\.checkpoint_dir, exist_ok=True\)
        
        checkpoint_file = f\"{self\.checkpoint_dir}/{checkpoint\.project_name}_{checkpoint\.checkpoint_id}\.json\""""

new_method = """    async def save_checkpoint(self, checkpoint) -> str:
        \"\"\"Save project checkpoint to disk\"\"\"
        import os
        os.makedirs(self.checkpoint_dir, exist_ok=True)
